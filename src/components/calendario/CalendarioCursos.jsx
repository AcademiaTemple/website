...
const Calendario = () => {
  ...
  const obtenerClasesPorFecha = () => {
    ...
    const clases = cursos.filter(curso =>
      (moment(fecha).isBetween(moment(curso.fInicioFin[0], 'DD/MM/YYYY'), moment(curso.fInicioFin[1], 'DD/MM/YYYY')) && // Verifico si está en el rango de inicio fin
  ...
  const IconoCalendario = ({ value, onClick }) => <i onClick={onClick} className="fa fa-calendar-alt"></i>;
                ...
                clasesActuales.map((e, i) => (
                  <div key={i} className="curso-calendario">
                    ...
                    <p className="horario-curso-calendario">{
                      moment(e.hInicioFin[0], 'HH:mm').format('hh:mm a') + ' - ' + moment(e.hInicioFin[1], 'HH:mm').format('hh:mm a')
                    }</p>
                  </div>
                ))
              }
